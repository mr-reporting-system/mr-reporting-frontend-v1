...
import {
  ...
  Stethoscope, Calendar as CalendarIcon, ArrowLeft, Search, X, User,
} from "lucide-react";
...
// ─── Column definitions from Doctor_Visit_Report.xlsx ───────────────────────
// S.No | State Name | Headquarter Name | Designation | Area Name | Employee Name
// Employee Code | Doctor Name | Code | Category | Specialization | Degree
// Address | Phone | Status | Frequency Visit | Total Visit | Visit Dates
// Product Name | POB | Order Qty | Sample Qty | reminder | Discussion
...
export default function DoctorVisitReport() {
  ...
  // ─── Filter State ─────────────────────────────────────────────────────────
  const [selectType, setSelectType]           = useState("");       // "state" | "headquarter"
  ...
  const [selectedEmployee, setSelectedEmployee]         = useState("");  // single
  ...
  // Employee drill-down (hierarchical)
  ...
  useEffect(() => {
    api.get("/api/masters/states", getAuth())
      .then(res => {
        ...
        setStatesList(
          Array.isArray(data)
            ? data.map(s => ({ id: String(s.id ?? s.stateId), label: s.state_name || s.stateName || s.name || "Unknown" }))
            : []
        );
      })
  ...
  useEffect(() => {
    api.get("/api/masters/designations", getAuth())
      .then(res => {
        ...
        setDesignationsList(
          Array.isArray(data)
            ? data.map(d => ({ id: String(d.id ?? d.designationId), label: d.designation_name || d.designationName || d.name || "Unknown" }))
            : []
        );
      })
  ...
  // ─── 3. Fetch Employees when designations selected (hierarchical) ─────────
  useEffect(() => {
    ...
    api.get(`/api/masters/employees?designationIds=${selectedDesignations.join(",")}`, getAuth())
      .then(res => {
        ...
        setEmployeesList(
          Array.isArray(data)
            ? data.map(e => ({ id: String(e.id ?? e.employeeId), label: e.employee_name || e.employeeName || e.name || "Unknown" }))
            : []
        );
      })
  ...
  // ─── Category options (static, from images) ──────────────────────────────
  ...
  const validate = () => {
    ...
    if (viewMode === "geographical" && selectType === "state" && selectedStates.length === 0) return "Please select at least one State.";
    if (viewMode === "hierarchical") {
      if (selectedDesignations.length === 0) return "Please select at least one Designation.";
  ...
  const handleViewReport = async () => {
    ...
    setSearch("");
    try {
      const res = await api.get(`/api/reports/doctor-visit?${buildParams()}`, getAuth());
      const data = res.data?.data || res.data || [];
  ...
  const handleEmployeeClick = async (row) => {
    ...
    try {
      ...
      const res = await api.get(`/api/reports/doctor-visit/employee-detail?${p}`, getAuth());
      const data = res.data?.data || res.data || [];
  ...
  // ─── Can generate? ────────────────────────────────────────────────────────
  const canGenerate = (() => {
    if (!selectType || !fromDate || !toDate) return false;
  ...
  const TABLE_COLS = [
    ...
    { key: "frequencyVisit",   label: "Freq. Visit",     center: true  },
    ...
    { key: "reminder",         label: "Reminder",        center: true  },
  ...
  return (
    <div style={{ width:"100%", paddingBottom:48, fontFamily:"Inter,sans-serif" }}>
      ...
      {/* ══ FILTER CARD ════════════════════════════════════════════════════ */}
      {/* Changed overflow:"hidden" to overflow:"visible" to allow datepicker dropdowns to show fully */}
      <div style={{ background:"#fff", borderRadius:16, boxShadow:"0 1px 4px rgba(0,0,0,0.07)", border:"1px solid #f3f4f6", overflow:"visible", marginBottom: 20 }}>
  ...
  <p style={{ fontSize: 11, color: "#6b7280", margin: 0, marginTop: 2 }}>
    Analyse doctor visits by geography or hierarchy
  </p>
</div>
          </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 250px)", gap:16, alignItems:"start" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <SingleDropdown
                    label="SELECT TYPE *"
                    value={selectType}
                    onSelect={v => { setSelectType(v); setSelectedStates([]); setShowTable(false); setValidationErr(""); }}
                    options={[
                ...
                {/* Select State (only for state type) */}
                {selectType === "state" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <MultiDropdown
                      label="SELECT STATE *"
            ...
            {viewMode === "hierarchical" && (
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16, alignItems:"start" }}>
                ...
                <MultiDropdown
                  ...
                  onChange={v => { setSelectedDesignations(v); setSelectedEmployee(""); }}
                />
      ...
      {showTable && (
        <div style={{ background:"#fff", borderRadius:16, border:"1px solid #f3f4f6", overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.07)" }}>
          ...
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", minWidth:"1600px", borderCollapse:"collapse", textAlign:"left", fontSize:13 }}>
              <thead style={{ background:"#f3f4f6", position:"sticky", top:0, zIndex:10 }}>
                <tr>
                  {TABLE_COLS.map((col, i) => (
                    <th key={i} style={{ padding:"12px 16px", fontWeight:700, color:"#374151", textTransform:"uppercase", letterSpacing:"0.05em", fontSize:11, textAlign: col.center ? "center" : "left", borderBottom:"1px solid #e5e7eb" }}>
                      {col.label}
                    </th>
                  ))}
                </tr>
                ...
                {!tableLoading && !drillLoading && !tableError && paged.map((row, idx) => (
                  ...
                  >
                    {TABLE_COLS.map((col, ci) => (
                      <td key={ci} style={{ padding:"10px 16px", textAlign: col.center ? "center" : "left", color:"#374151" }}>
                        {col.key === "sNo" ? (
                          <span style={{ fontWeight:600, color:"#9ca3af" }}>{(activePage - 1) * PAGE_SIZE + idx + 1}</span>
...
function CategoryBadge({ value }) {
  const map = {
    "A+":   { bg: "#f3e8ff", color: "#7e22ce", border: "#e9d5ff" },
    "A":    { bg: "#dbeafe", color: "#1d4ed8", border: "#bfdbfe" },
    "B+":   { bg: "#e0f2fe", color: "#0369a1", border: "#bae6fd" },
    "B":    { bg: "#ccfbf1", color: "#0f766e", border: "#99f6e4" },
    "C":    { bg: "#fef3c7", color: "#b45309", border: "#fde68a" },
    "D":    { bg: "#f1f5f9", color: "#475569", border: "#e2e8f0" },
    "Core": { bg: "#ffe4e6", color: "#be123c", border: "#fecdd3" },
  };
  const style = map[String(value)] || { bg: "#f1f5f9", color: "#64748b", border: "#e2e8f0" };
  return <span style={{ padding:"2px 6px", borderRadius:4, fontSize:10, fontWeight:700, textTransform:"uppercase", background: style.bg, color: style.color, border: `1px solid ${style.border}` }}>{value ?? "—"}</span>;
}

function StatusBadge({ value }) {
  const isTrue  = value === true  || String(value).toLowerCase() === "true"  || String(value).toLowerCase() === "active";
  const isFalse = value === false || String(value).toLowerCase() === "false" || String(value).toLowerCase() === "inactive";
  if (isTrue)  return <span style={{ padding:"2px 6px", borderRadius:4, fontSize:10, fontWeight:700, textTransform:"uppercase", background:"#dcfce7", color:"#15803d", border:"1px solid #bbf7d0" }}>Active</span>;
  if (isFalse) return <span style={{ padding:"2px 6px", borderRadius:4, fontSize:10, fontWeight:700, textTransform:"uppercase", background:"#fee2e2", color:"#b91c1c", border:"1px solid #fecaca" }}>Inactive</span>;
  return <span style={{ color:"#6b7280", fontSize:12 }}>{String(value ?? "—")}</span>;
}
