...
import {
  ...
  ShoppingCart, ArrowLeft, Search, X, Download,
...
const TABLE_COLS = [
  ...
  { key: "statusDated",                label: "Status Dated",              center: true  },
  { key: "designation",                label: "Designation",               center: true  },
  { key: "dateOfJoining",              label: "Date Of Joining",           center: true  },
  { key: "workingDays",                label: "Working Days",              center: true  },
  { key: "tc",                         label: "TC",                        center: true  },
  { key: "pc",                         label: "PC",                        center: true  },
  { key: "pob",                        label: "POB",                       center: true  },
  { key: "visitingArea",               label: "Visiting Area"                            },
  { key: "stockistForwarded",          label: "Stockist Forwarded",        center: true  },
  { key: "sob",                        label: "SOB",                       center: true  },
  { key: "progressiveCalls",           label: "Progressive Calls",         center: true  },
  { key: "progressiveProductiveCalls", label: "Progressive Prod. Calls",   center: true  },
  { key: "totalPOB",                   label: "Total POB",                 center: true  },
  { key: "stockistDetails",            label: "Stockist Details"                         },
  { key: "totalSOB",                   label: "Total SOB",                 center: true  },
];

export default function POBReportStockistWise() {
  ...
  const [geoStatus,         setGeoStatus]        = useState("active");
  const [geoFromDate,       setGeoFromDate]      = useState("");
  const [geoToDate,         setGeoToDate]        = useState("");

  const [hierType,     setHierType]     = useState("");
  const [hierStatus,   setHierStatus]   = useState("active");
  ...
  useEffect(() => {
    if (geoType !== "headquarter" || selectedStates.length === 0) {
      setDistrictsList([]); setSelectedDistricts([]);
      return;
    }
  ...
  }, [selectedStates, geoType, getAuth]);

  const switchMode = (mode) => {
    setViewMode(mode);
    setGeoType(""); setSelectedStates([]); setSelectedDistricts([]);
    setGeoStatus("active"); setGeoFromDate(""); setGeoToDate("");
    setHierType(""); setHierStatus("active"); setHierFromDate(""); setHierToDate("");
  ...
  const validate = () => {
    if (viewMode === "geographical") {
      ...
      if (geoType === "headquarter" && selectedDistricts.length === 0) return "Headquarter is required!!!";
      if (!geoStatus) return "Status is required!!!";
      if (!geoFromDate) return "From Date is required!!!";
      if (!geoToDate)   return "To Date is required!!!";
      if (geoFromDate > geoToDate) return "From Date cannot be after To Date.";
    }
    if (viewMode === "hierarchical") {
      if (!hierType) return "Type is required!!!";
      if (!hierStatus) return "Status is required!!!";
  ...
  const buildParams = () => {
    ...
    if (viewMode === "geographical") {
      ...
      p.append("status",   geoStatus);
      p.append("fromDate", geoFromDate);
      p.append("toDate",   geoToDate);
  ...
  const handleViewRecord = async () => {
    ...
    setPage(1); setSearch("");
    try {
      const res = await api.get(`/api/reports/pob-stockist-wise?${buildParams()}`, getAuth());
  ...
  const handleDownload = () => {
    ...
    a.download = "POB_Report_Stockist_Wise.csv";
    a.click();
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-visible">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>
                POB Report Stockist Wise
              </h2>
              <p style={{ fontSize: 11, color: "#6b7280", margin: 0, marginTop: 2 }}>
                Analyse stockist-wise POB by geography or hierarchy
              </p>
            </div>
          </div>
        ...
        {filterOpen && (
          <div className="p-6 sm:p-8 space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-5 gap-y-6 items-start">
                  ...
                  <div className="space-y-1">
                    <SingleDropdown
                      label="SELECT TYPE *"
                      value={geoType}
                      onSelect={v => { setGeoType(v); setSelectedStates([]); setSelectedDistricts([]); setValidationErr(""); }}
                      options={[
                  ...
                  {geoType === "headquarter" && (
                    <div className="space-y-1">
                      ...
                      />
                      {validationErr === "Headquarter is required!!!" && (
                        <p style={{ fontSize: 10, fontWeight: 700, color: "#ef4444", paddingTop: 2 }}>Headquarter is required!!!</p>
                      )}
                    </div>
                  )}
                  ...
                  <SingleDropdown
                    label="SELECT STATUS *"
                    value={geoStatus}
                    onSelect={v => { setGeoStatus(v); setValidationErr(""); }}
                    options={STATUS_OPTS}
                  />

                  <FDatePicker label="FROM DATE" value={geoFromDate} onChange={setGeoFromDate} />
                  <FDatePicker label="TO DATE"   value={geoToDate}   onChange={setGeoToDate}   />
                </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-5 gap-y-6 items-start">
                  ...
                  <SingleDropdown
                    label="SELECT STATUS *"
                    value={hierStatus}
                    onSelect={v => { setHierStatus(v); setValidationErr(""); }}
                    options={STATUS_OPTS}
                  />
            ...
            {validationErr && !["Type is required!!!", "State is required!!!", "Headquarter is required!!!"].includes(validationErr) && (
                    ...
                    >
                      {TABLE_COLS.map(col => (
                        ...
                        >
                          ...
                          ) : col.key === "visitingArea" || col.key === "stockistDetails" ? (
                            <span className="whitespace-normal max-w-[200px] inline-block leading-relaxed">
                              {row[col.key] ?? "—"}
                            </span>
                          ) : (
                            row[col.key] ?? "—"
      ...
      {open && !disabled && (
        <div style={{
          ...
          <div style={{ display: "flex", borderBottom: "1px solid #f3f4f6" }}>
            <button onClick={(e) => { e.preventDefault(); selectAll(); }} style={{ flex: 1, padding: "8px", fontSize: 11, fontWeight: 700, color: "#fff", background: "#2563eb", border: "none", cursor: "pointer" }}>Select All</button>
            <button onClick={(e) => { e.preventDefault(); clearAll(); }} style={{ flex: 1, padding: "8px", fontSize: 11, fontWeight: 700, color: "#fff", background: "#ef4444", border: "none", cursor: "pointer" }}>Deselect All</button>
          </div>
