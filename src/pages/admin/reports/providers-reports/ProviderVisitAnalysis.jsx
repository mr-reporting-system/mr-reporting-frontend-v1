...
import {
  ...
  BarChart2, Search, X, Check, Download,
...
const YEARLY_OPTIONS = Array.from({ length: 6 }, (_, i) => {
  const y = CURRENT_YEAR - 2 + i;
  return { value: String(y), label: String(y) };
});
const FINANCIAL_YEAR_OPTIONS = Array.from({ length: 4 }, (_, i) => {
...
const TABLE_COLS = [
  ...
  { key: "providerStatus", label: "Provider Status", center: true  },
  { key: "areaStatus",     label: "Area Status",     center: true  },
];

export default function ProviderVisitAnalysisReport() {
  // ─── UI ────────────────────────────────────────────────────────────────────
  const [filterOpen,    setFilterOpen]    = useState(true);
  const [reportMode,    setReportMode]    = useState("yearly"); // yearly | financialYearly | hierarchyWise
  ...
  const [teamType,      setTeamType]      = useState(""); // withTeam | withoutTeam
  const [selectedTeams, setSelectedTeams] = useState([]); // multi (shown when teamType = withTeam)
  ...
  // ─── 2. Employees when designation or status changes ───────────────────────
  useEffect(() => {
    const params = new URLSearchParams();
    if (designation) params.append("designationId", designation);
    if (status)      params.append("status",        status);
    api.get(`/api/masters/employees?${params}`, getAuth())
      .then(res => {
        const d = res.data?.data || res.data || [];
  ...
  }, [designation, status, getAuth]);

  // ─── 3. Teams when employee + teamType = withTeam ─────────────────────────
  useEffect(() => {
    if (!employee || teamType !== "withTeam") { setTeamsList([]); setSelectedTeams([]); return; }
    api.get(`/api/masters/teams?employeeId=${employee}`, getAuth())
      .then(res => {
        const d = res.data?.data || res.data || [];
        setTeamsList(Array.isArray(d) ? d.map(t => ({
          id:    String(t.id ?? t.teamId),
          label: t.team_name || t.teamName || t.name || "Unknown",
        })) : []);
      })
      .catch(e => console.error("teams", e));
    setSelectedTeams([]);
  }, [employee, teamType, getAuth]);
  ...
  const validate = () => {
    ...
    if (!employee) return "Employee name is required!!!";
    if (reportMode === "yearly"          && !selectedYear)   return "Year is required!!!";
    if (reportMode === "financialYearly" && !financialYear)  return "Financial Year is required!!!";
    if (reportMode === "hierarchyWise"   && !fromDate)       return "From Date is required!!!";
    if (reportMode === "hierarchyWise"   && !toDate)         return "To Date is required!!!";
    if (reportMode === "hierarchyWise"   && fromDate > toDate) return "From Date cannot be after To Date.";
    if (providerRadio === "doctor" && selectedCats.length === 0) return "Category is required!!!";
    return "";
  };
  ...
  const buildParams = () => {
    ...
    p.append("mode",         reportMode);
    p.append("providerType", providerRadio);
    if (designation)            p.append("designationId",  designation);
    if (status)                 p.append("status",         status);
    if (employee)               p.append("employeeId",     employee);
    if (teamType)               p.append("teamType",       teamType);
    if (selectedTeams.length)   p.append("teamIds",        selectedTeams.join(","));
    if (selectedCats.length)    p.append("categories",     selectedCats.join(","));
    if (reportMode === "yearly")          p.append("year",          selectedYear);
    if (reportMode === "financialYearly") p.append("financialYear", financialYear);
    if (reportMode === "hierarchyWise") {
  ...
  const handleViewReport = async () => {
    ...
    setPage(1); setSearch("");
    try {
      const res = await api.get(`/api/reports/provider-visit-analysis?${buildParams()}`, getAuth());
  ...
  const handleDownload = () => {
    ...
    a.download = "Provider_Visit_Analysis_Report.csv";
    a.click();
  };
  ...
  // ─── Team type options ─────────────────────────────────────────────────────
  const TEAM_TYPE_OPTS = [
    { value: "withTeam",    label: "With Team"    },
    { value: "withoutTeam", label: "Without Team" },
  ];
  ...
  // ─── Shared filter grid (common to all 3 modes) ────────────────────────────
    ...
    <>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-5 gap-y-6 items-start">
        ...
        <SingleDropdown
          ...
          value={designation}
          onSelect={v => { setDesignation(v); setValidationErr(""); }}
          options={designationsList.length ? designationsList : [
            { value: "ASM", label: "ASM" }, { value: "MR",  label: "MR"  },
            { value: "NSM", label: "NSM" }, { value: "RSM", label: "RSM" },
            { value: "ZSM", label: "ZSM" },
          ]}
        />
        ...
        <SingleDropdown
          label="SELECT STATUS *"
          value={status}
          onSelect={v => { setStatus(v); setValidationErr(""); }}
          options={STATUS_OPTS}
        />
        ...
        <div className="space-y-1">
          <SingleDropdown
            ...
            value={employee}
            onSelect={v => { setEmployee(v); setValidationErr(""); }}
            options={employeesList}
          />
          {validationErr === "Employee name is required!!!" && (
            <p style={{ fontSize: 10, fontWeight: 700, color: "#ef4444", paddingTop: 2 }}>Employee name is required!!!</p>
          )}
        </div>
        ...
        <SingleDropdown
          ...
          value={teamType}
          onSelect={v => { setTeamType(v); setSelectedTeams([]); setValidationErr(""); }}
          options={TEAM_TYPE_OPTS}
        />

        {/* Select Team (multi, only when withTeam) */}
        {teamType === "withTeam" && (
          <MultiDropdown
            label="SELECT TEAM"
            options={teamsList}
        ...
        {/* Year / Financial Year / Date fields per mode */}
        {reportMode === "yearly" && (
          <SingleDropdown
            label="SELECT YEAR *"
            value={selectedYear}
        ...
        {reportMode === "financialYearly" && (
          <SingleDropdown
            label="SELECT FINANCIAL YEAR *"
            value={financialYear}
            onSelect={v => { setFinancialYear(v); setValidationErr(""); }}
            options={FINANCIAL_YEAR_OPTIONS}
          />
        )}
        {reportMode === "hierarchyWise" && (
          <>
            <MultiDropdown
              label="SELECT CATEGORY *"
              options={CATEGORY_OPTIONS}
              selectedIds={selectedCats}
              onChange={v => { setSelectedCats(v); setValidationErr(""); }}
            />
        ...
        <div className="flex flex-col gap-2 justify-center mt-2">
          <ProviderRadio value={providerRadio} onChange={v => { setProviderRadio(v); setSelectedCats([]); }} />
        </div>

        {/* Category (yearly + financialYearly, when doctor) */}
        {reportMode !== "hierarchyWise" && providerRadio === "doctor" && (
          <div className="space-y-1">
            <SingleDropdown
              label="SELECT CATEGORY *"
              value={selectedCats[0] || ""}
              onSelect={v => { setSelectedCats(v ? [v] : []); setValidationErr(""); }}
              options={CATEGORY_OPTIONS.map(c => ({ value: c.id, label: c.label }))}
            />
            {validationErr === "Category is required!!!" && (
              <p style={{ fontSize: 10, fontWeight: 700, color: "#ef4444", paddingTop: 2 }}>Category is required!!!</p>
            )}
          </div>
        )}
      </div>
      ...
      {validationErr && validationErr !== "Employee name is required!!!" && validationErr !== "Category is required!!!" && (
  ...
  <h2 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>
    Provider Visit Analysis Report
  </h2>
  <p style={{ fontSize: 11, color: "#6b7280", margin: 0, marginTop: 2 }}>
    Analyse provider visits yearly, financially or by hierarchy
  </p>
</div>
          </div>
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            <div className="flex items-center gap-8">
              {[
                ...
                { val: "financialYearly", label: "Financial Yearly" },
                { val: "hierarchyWise",   label: "Hierarchy Wise"   },
              ].map(opt => (
                <label key={opt.val} className="flex items-center gap-2 cursor-pointer group" onClick={() => switchMode(opt.val)}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${reportMode === opt.val ? "border-blue-600" : "border-slate-300 group-hover:border-blue-300"}`}>
                    {reportMode === opt.val && <div className="w-2 h-2 rounded-full bg-blue-600" />}
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700, textTransform: "capitalize", color: reportMode === opt.val ? "#111827" : "#4b5563" }}>
                    {opt.label}
                  </span>
                </label>
              ))}
          ...
          <div className="px-6 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/40">
            <div className="flex items-center gap-2">
              <span style={{ fontSize: 13, fontWeight: 700, color: "#111827" }}>
                Provider Visit Analysis Details
              </span>
                    ...
                    >
                      {TABLE_COLS.map(col => (
                        ...
                        >
                          ...
                          ) : col.key === "providerStatus" || col.key === "areaStatus" ? (
  ...
  return (
    <div className="flex items-center gap-8">
      {[
        ...
        { val: "chemist", label: "Chemist/ Stockist" },
      ].map(opt => (
...
function CategoryBadge({ value }) {
  const map = {
    ...
    "Core": "bg-rose-100 text-rose-700",   "SuperCore": "bg-pink-100 text-pink-700",
  };
  return (
    <span className={`px-2 py-0.5 rounded font-bold text-[10px] ${map[String(value)] || "bg-slate-100 text-slate-500"}`}>
      {value ?? "—"}
    </span>
  );
}
