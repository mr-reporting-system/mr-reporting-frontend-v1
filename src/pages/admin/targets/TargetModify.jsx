...
export default function TargetModify() {
  ...
  const [filters, setFilters] = useState({
    stateId: "",
    districtIds: [],
    employeeId: "", // Changed to Singular based on backend API contract
  ...
  useEffect(() => {
    setFilters(prev => ({ ...prev, districtIds: [], employeeId: "" }));
  ...
  const handleFetchTargetData = async () => {
    ...
    if (!filters.stateId || filters.districtIds.length === 0 || !filters.employeeId || !filters.year) {
      return setError("Please select State, District, Employee, and Year to load existing targets.");
    }

    setIsLoading(true);
    try {
      const res = await api.get(`/api/target/submission/modify/form?employeeId=${filters.employeeId}&year=${filters.year}`, getAuthHeaders());
      
      const fetchedData = res.data?.data?.monthlyTargets || {};
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await api.put("/api/target/submission/modify", payload, getAuthHeaders());
      
      setSuccessMsg("Targets modified successfully!");
      setTimeout(() => {
        setSuccessMsg("");
        setTableVisible(false);
      }, 3500);
  ...
  const stateOpts = states.map(s => ({ id: s.id, value: s.id, label: s.stateName }));
  const distOpts = districts.map(d => ({ id: d.id, value: d.id, label: d.districtName }));
  const empOpts = employees.map(e => ({ id: e.id, value: e.id, label: e.employeeName }));
    ...
    <div className="ucr-wrap">
      ...
      <div className="ucr-card">
        <div className="ucr-header">
          ...
          <div style={{ flex: 1 }}>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: "#111827", margin: 0 }}>Target Modify</h2>
            <p style={{ fontSize: 11, color: "#6b7280", margin: 0, marginTop: 2 }}>Adjust and update existing monthly targets</p>
          </div>
        ...
        {isFilterOpen && (
          <div className="ucr-body">
            <div className="ucr-grid-4">
              ...
              <SingleDropdown label="SELECT EMPLOYEE *" options={empOpts} value={filters.employeeId} onSelect={(v) => handleFilterChange("employeeId", v)} disabled={!filters.districtIds.length} />
              ...
              <div>
                ...
                >
                  {isLoading ? <Loader2 size={16} style={{ animation: "ucr-spin 1s linear infinite" }} /> : <Edit size={16} />}
                  Modify Target
                </button>
              </div>
              <div style={{ gridColumn: "span 2" }} className="hide-on-mobile"></div>
      ...
      {tableVisible && (
        <div className="ucr-card animate-in slide-in-from-bottom-4 duration-500">
          <div className="ucr-header" style={{ background: "#f9fafb" }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: "#374151", margin: 0 }}>Target Modify Form</h3>
          </div>
          ...
          <div className="ucr-footer" style={{ justifyContent: "flex-start", borderTop: "1px solid #f3f4f6" }}>
            ...
            >
              {isSubmitting ? <Loader2 size={16} style={{ animation: "ucr-spin 1s linear infinite" }} /> : <Check size={16} />}
              Modify
            </button>
          </div>
        </div>
